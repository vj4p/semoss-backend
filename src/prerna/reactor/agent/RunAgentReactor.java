...
/**
 * Starts a durable generic agent run and waits by default unless wait=false is
 * passed.
 */
public class RunAgentReactor extends AbstractReactor {
	...
	public RunAgentReactor() {
		this.keysToGet = new String[] { ReactorKeysEnum.ROOM_ID.getKey(), ReactorKeysEnum.COMMAND.getKey(),
				ReactorKeysEnum.ENGINE.getKey(), HARNESS_TYPE_KEY, WORKSPACE_ID_KEY, MAX_TURNS_KEY, MAX_REFLECTIONS_KEY,
				WAIT_KEY, WAIT_TIMEOUT_MS_KEY, ReactorKeysEnum.PARAM_VALUES_MAP.getKey(),
				ReactorKeysEnum.AGENT_PARAMS.getKey(), MEDIA_KEY, ReactorKeysEnum.URL.getKey(), };
	...
	public NounMetadata execute() {
		...
		int maxReflections = parseIntAtLeast(this.keyValue.get(MAX_REFLECTIONS_KEY),
		...
		try {
			...
			AgentRunRequest request = new AgentRunRequest(roomId, input, engineIdFallback, harnessType,
					...
					this.insight);
			AgentRunHandle handle = AgentRunService.get().run(request);
			Map<String, Object> output = handle.toMap();
			if (wait) {
				output = AgentRunService.get().waitForRun(handle.runId(), this.insight, waitTimeoutMs);
			}
			// async submit handle; terminal output (when wait=true) comes from waitForRun
			// above
			logger.info("RunAgentReactor: runId={} status={}", handle.runId(), handle.status());
	...
	private List<String> stageMediaInputs(String roomId, String input, String engineIdFallback,
			List<String> inputMedia) {
	...
	private void validateMediaSupported(IAgentHarness harness, List<String> inputMedia, List<String> inputMediaURLs) {
	...
	/**
	 * Parse {@code value} as an int, falling back to {@code defaultValue} when
	 * null, blank, non-numeric, or below {@code minInclusive}.
	 */
	...
	protected MCP_KEY_TYPE getKeyTypeForMCP(String key) {
		...
		if (key.equals(ReactorKeysEnum.PARAM_VALUES_MAP.getKey())
				|| key.equals(ReactorKeysEnum.AGENT_PARAMS.getKey())) {
